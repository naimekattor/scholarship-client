...
const AddScholarship = () => {
  ...
  const mutation = useMutation({
    mutationFn: (newScholarship) =>
      axiosSecure.post("/scholarships", newScholarship),
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error(
        error.response?.data?.message || "Failed to add scholarship."
      );
    },
  });
      ...
      if (imgbbResponse.data.success) {
        const scholarshipData = {
          ...
          universityWorldRank: data.universityWorldRank
            ? parseInt(data.universityWorldRank)
            : undefined,
          tuitionFees: data.tuitionFees
            ? parseFloat(data.tuitionFees)
            : undefined,
        };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <Input
              id="universityLogo"
              type="file"
              {...register("universityLogo", { required: true })}
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
